...
class Gomoku_Config:
    ...
    #----------环境相应参数------------
        ...
        self.env_id = "gomoku"
        self.board_size = 11
        self.encode_state_channels = 12  # 己方：位置 、1连棋、2连棋、3连棋、4及以上连棋 ；
                                         # 对方：位置 、1连棋、2连棋、3连棋、4及以上连棋 ； 上个落子点； 上上落子点
                 
        self.black = 1
        self.white = 2  

    #----------储存区参数---------------
        self.replay_buffer_size = 12000  # 局数  
        self.is_save_buffer = False   # 
        self.store_batch = 5         # 保存5份数据
    ...
    #----------自对弈参数----------------
        ...
        self.total_game_num = 1e8   # 总对弈局数
        self.play_workers_num = 7
        self.c_puct = 3             # 3 是比较中性的值；  
                                   
        self.num_simulation= 365
        self.action_space = list(range(self.board_size**2)) # 动作空间, 棋盘各点+停着
    ...
    #----------其他----------------------
        ...
        self.results_path = "./results/gomoku"   
        self.record_train = "train_record.txt"
        self.record_summarywriter_path = "runs1/loss1"
